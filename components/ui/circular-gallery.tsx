...
function autoBind(instance: object) {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof (instance as Record<string, unknown>)[key] === "function") {
      (instance as Record<string, unknown>)[key] = (instance as Record<string, unknown>)[key] as (...a: unknown[]) => unknown;
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor, font }: { gl: OGLRenderingContext; plane: Mesh; renderer: Renderer; text: string; textColor: string; font: string }) {
    autoBind(this);
...
class App {
  ...
  createMedias(items: GalleryItem[] | undefined, bend: number, textColor: string, borderRadius: number, font: string) {
    const defaultItems: GalleryItem[] = [
      { image: "https://images.unsplash.com/photo-1522120691812-dcdfb625f397?q=80&w=800&auto=format&fit=crop", text: "Cue" },
    ];
...
const CircularGallery = ({ items, bend = 3, borderRadius = 0.05, scrollSpeed = 2, scrollEase = 0.05, className, fontClassName, ...props }: CircularGalleryProps) => {
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, { items, bend, textColor: computedColor, borderRadius, font: computedFont, scrollSpeed, scrollEase });
    return () => { app.destroy(); };
